...
use super::{ByteStream, values::{uxx, vec_, name, vec_of_byte}, types::{functype, valtype}, instructions::expr};

use std::iter::{zip, repeat};

pub fn section<'a, I: 'a + ByteStream<'a>>(n: u8) -> impl Fn(I) -> IResult<I, Option<I>, I::Error> {
    move |i: I| {
...
fn custom_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Custom<I>, I::Error> {
    let (i, name) = name(i)?;
    let (end, i) = take(i.input_len())(i)?;
    Ok((end, Custom { name, content: i }))
}

fn type_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<FuncType>, I::Error> {
    let (i, fts) = vec_(functype)(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, fts)) 
}

fn importdesc<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, ImportDesc, I::Error> {
    alt((
...
fn import<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Import, I::Error> {
    map(
...
fn import_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Import>, I::Error> {
    let (i, imports) = vec_(import)(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, imports)) 
}

fn function_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<TypeIdx>, I::Error> {
    let (i, functions) = vec_(uxx::<u32, I>)(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, functions)) 
}

fn table_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Table>, I::Error> {
...
fn mem_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Mem>, I::Error> {
    let (i, mems) = vec_(map(memtype, |tpe| Mem { tpe }))(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, mems)) 
}

fn global_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Global>, I::Error> {
    let (i, mems) = vec_(map(pair(globaltype, expr), |(tpe, expr)| Global { tpe, expr }))(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, mems)) 
}

fn exportdesc<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, ExportDesc, I::Error> {
    alt((
...
fn export<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Export, I::Error> {
    map(
...
fn export_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Export>, I::Error> {
    let (i, mems) = vec_(export)(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, mems)) 
}

fn start_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Option<Start>, I::Error> {
    let (i, start) = uxx::<u32, I>(i)?;
    let (i, _) = eof!(i.clone(),)?;
    Ok((i, Some(Start(start))))
}

fn element<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Element, I::Error> {
...
fn element_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Element>, I::Error> {
...
fn code<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Code, I::Error> {
    let (i, size) = uxx(i)?;
    let (i, j) = take(size)(i)?;
...
fn code_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Code>, I::Error> {
...
fn data<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Data, I::Error> {
...
fn data_section<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Vec<Data>, I::Error> {
...
pub fn module<'a, I: 'a + ByteStream<'a>>(i: I) -> IResult<I, Module, I::Error>
{
