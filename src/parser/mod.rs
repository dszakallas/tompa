use nom::{
    ...
    sequence::pair,
    sequence::tuple,
    tag, take_while, take_while1, tuple, AsChar as NomAsChar, Compare, IResult, InputIter,
    InputLength, InputTake, InputTakeAtPosition, Slice,
};
use num::{Float, Num, Signed, Unsigned};
use std::fmt::Display;
use std::io::Write;
...
#[inline]
fn id<I, E: ParseError<I>>(i: I) -> IResult<I, I, E>
where
...
#[inline]
fn idchar<I, E: ParseError<I>>(i: I) -> IResult<I, I, E>
where
    ...
    <I as InputTakeAtPosition>::Item: NomAsChar,
{
    ...
    match hex {
        Some(_) => map_res(
            |i: I| num(i, map(hex_digit1, |i: I| i.as_str())),
            |s| <Out as Num>::from_str_radix(s.as_str(), 16),
        )(i),
        None => map_res(
            |i: I| num(i, map(digit1, |i: I| i.as_str())),
...
#[inline]
fn signed_int<'a, I: 'a, E: ParseError<I>, Out: Signed>(i: I) -> IResult<I, Out, E>
where
    I: Clone
        + PartialEq
        + Slice<RangeFrom<usize>>
...
#[inline]
fn signed_int_with_sign<'a, I: 'a, E: ParseError<I>, Out: Signed>(
    i: I,
    sign: char,
) -> IResult<I, Out, E>
where
    ...
    <I as InputTakeAtPosition>::Item: NomAsChar,
{
    ...
    match hex {
        Some(_) => map_res(
            |i: I| num(i, map(hex_digit1, |i: I| i.as_str())),
            move |s| <Out as Num>::from_str_radix(format!("{}{}", sign, s.as_str()).as_str(), 16),
        )(i),
        None => map_res(
            |i: I| num(i, map(digit1, |i: I| i.as_str())),
            move |s| <Out as Num>::from_str_radix(format!("{}{}", sign, s.as_str()).as_str(), 10),
        )(i),
    }
}
    ...
    <Out as FromSigned>::Repr: Signed,
{
    ...
    match sign {
        Some(sign) => map(
            |i| signed_int_with_sign::<'a, I, E, <Out as FromSigned>::Repr>(i, sign),
            |n| FromSigned::get(n),
        )(i),
